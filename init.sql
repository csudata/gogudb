/* ------------------------------------------------------------------------
 *
 *	Creates schema, config table and provides common utility functions
 *
 * ------------------------------------------------------------------------
 */
...
CREATE SCHEMA IF NOT EXISTS gogudb_partition_table;
...
CREATE OR REPLACE FUNCTION @extschema@.validate_interval_value(
	...
	range_interval	TEXT,
	cooked_expr		TEXT)
RETURNS BOOL AS 'MODULE_PATHNAME', 'validate_interval_value'
LANGUAGE C;
 ...
 */
CREATE TABLE IF NOT EXISTS @extschema@.gogudb_config (
	...
	CONSTRAINT gogudb_config_parttype_check CHECK (parttype IN (1, 2)),

	/* check for correct interval */
	CONSTRAINT gogudb_config_interval_check
...
CREATE TABLE IF NOT EXISTS @extschema@.server_map(
	server_name              TEXT NOT NULL,
	range_start		smallint NOT NULL,
	range_end		smallint NOT NULL,
	CONSTRAINT server_map_range_start_check CHECK (range_start >= 0 and range_start <= 127),
	CONSTRAINT server_map_range_end_check CHECK (range_end >= range_start and range_end <=128)
);

CREATE TABLE IF NOT EXISTS @extschema@.table_partition_rule(
	schema_name		TEXT NOT NULL,
	table_name              TEXT NOT NULL,
	part_expr               TEXT NOT NULL,
	part_type               INTEGER NOT NULL,
	/* check for allowed part types */
	CONSTRAINT table_partition_parttype_check CHECK (part_type IN (1, 2)),

	range_interval  TEXT DEFAULT NULL,
	range_start     TEXT DEFAULT NULL,
	part_dist       INTEGER NOT NULL,
	CONSTRAINT table_partition_part_dist_check CHECK ((part_type = 2) or (part_dist >= 1 and part_dist <= 128)),
...
RETURNS BOOL AS 'MODULE_PATHNAME', 'validate_part_callback_pl'
LANGUAGE C STRICT;
 ...
 */
CREATE TABLE IF NOT EXISTS @extschema@.gogudb_config_params (
	partrel			REGCLASS NOT NULL PRIMARY KEY,
...
ON @extschema@.gogudb_config, @extschema@.gogudb_config_params, @extschema@.table_partition_rule
TO public;
...
RETURNS BOOL AS 'MODULE_PATHNAME', 'check_security_policy' LANGUAGE C STRICT;

/*
 * Row security policy to restrict partitioning operations to owner and superusers only
 */
CREATE POLICY deny_modification ON @extschema@.gogudb_config
FOR ALL USING (check_security_policy(partrel));

CREATE POLICY deny_modification ON @extschema@.gogudb_config_params
FOR ALL USING (check_security_policy(partrel));

/*
CREATE POLICY deny_modification ON @extschema@.table_partition_rule
FOR ALL USING (check_security_policy(partrel));
*/
CREATE POLICY allow_select ON @extschema@.gogudb_config FOR SELECT USING (true);

CREATE POLICY allow_select ON @extschema@.gogudb_config_params FOR SELECT USING (true);

CREATE POLICY allow_select ON @extschema@.table_partition_rule FOR SELECT USING (true);

ALTER TABLE @extschema@.gogudb_config ENABLE ROW LEVEL SECURITY;
ALTER TABLE @extschema@.gogudb_config_params ENABLE ROW LEVEL SECURITY;
ALTER TABLE @extschema@.table_partition_rule ENABLE ROW LEVEL SECURITY;

/*
 * Invalidate relcache every time someone changes parameters config.
 */
CREATE OR REPLACE FUNCTION @extschema@.gogudb_config_params_trigger_func()
RETURNS TRIGGER AS 'MODULE_PATHNAME', 'gogudb_config_params_trigger_func'
LANGUAGE C;

CREATE TRIGGER gogudb_config_params_trigger
AFTER INSERT OR UPDATE OR DELETE ON @extschema@.gogudb_config_params
FOR EACH ROW EXECUTE PROCEDURE @extschema@.gogudb_config_params_trigger_func();

/*
 * Enable dump of config tables with pg_dump.
 */
SELECT pg_catalog.pg_extension_config_dump('@extschema@.gogudb_config', '');
SELECT pg_catalog.pg_extension_config_dump('@extschema@.gogudb_config_params', '');
SELECT pg_catalog.pg_extension_config_dump('@extschema@.table_partition_rule', '');


/*
 * Add a row describing the optional parameter to gogudb_config_params.
 */
CREATE OR REPLACE FUNCTION @extschema@.gogudb_set_param(
	relation	REGCLASS,
	param		TEXT,
	value		ANYELEMENT)
RETURNS VOID AS $$
BEGIN
	EXECUTE format('INSERT INTO @extschema@.gogudb_config_params
...
RETURNS VOID AS $$
BEGIN
	PERFORM @extschema@.gogudb_set_param(relation, 'enable_parent', value);
END
...
RETURNS VOID AS $$
BEGIN
	PERFORM @extschema@.gogudb_set_param(relation, 'auto', value);
END
$$ LANGUAGE plpgsql STRICT;
...
BEGIN
	...
	PERFORM @extschema@.gogudb_set_param(relation, 'init_callback', regproc_text);
END
$$ LANGUAGE plpgsql STRICT;
...
RETURNS VOID AS $$
BEGIN
	PERFORM @extschema@.gogudb_set_param(relation, 'spawn_using_bgw', value);
END
$$ LANGUAGE plpgsql STRICT;
...
DECLARE
	affected	INTEGER;
BEGIN
	UPDATE @extschema@.gogudb_config
...
RETURNS TABLE (
	...
	range_min		TEXT,
	range_max		TEXT)
AS 'MODULE_PATHNAME', 'show_partition_list_internal'
LANGUAGE C STRICT;

/*
 * View for show_partition_list().
 */
CREATE OR REPLACE VIEW @extschema@.gogudb_partition_list
AS SELECT * FROM @extschema@.show_partition_list();

GRANT SELECT ON @extschema@.gogudb_partition_list TO PUBLIC;

/*
 * Show memory usage of gogudb's caches.
 */
...
RETURNS TABLE (
	...
	used			INT8,
	entries			INT8)
AS 'MODULE_PATHNAME', 'show_cache_stats_internal'
LANGUAGE C STRICT;

/*
 * View for show_cache_stats().
 */
CREATE OR REPLACE VIEW @extschema@.gogudb_cache_stats
...
RETURNS TABLE (
	...
	processed	INT,
	status		TEXT)
AS 'MODULE_PATHNAME', 'show_concurrent_part_tasks_internal'
LANGUAGE C STRICT;

/*
 * View for show_concurrent_part_tasks().
 */
CREATE OR REPLACE VIEW @extschema@.gogudb_concurrent_part_tasks
AS SELECT * FROM @extschema@.show_concurrent_part_tasks();

GRANT SELECT ON @extschema@.gogudb_concurrent_part_tasks TO PUBLIC;

CREATE OR REPLACE VIEW @extschema@.fdw_server
AS SELECT server.srvname
   FROM pg_foreign_server server,
    pg_foreign_data_wrapper wrapper
  WHERE server.srvfdw = wrapper.oid AND wrapper.fdwname = 'gogudb_fdw'::name;

GRANT SELECT ON @extschema@.fdw_server TO PUBLIC;
/*
...
RETURNS VOID AS 'MODULE_PATHNAME', 'partition_table_concurrently'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.stop_concurrent_part_task(
	relation		REGCLASS)
RETURNS BOOL AS 'MODULE_PATHNAME', 'stop_concurrent_part_task'
LANGUAGE C STRICT;
...
/*
 * Disable gogudb partitioning for specified relation.
 */
CREATE OR REPLACE FUNCTION @extschema@.disable_gogudb_for(
	parent_relid	REGCLASS)
RETURNS VOID AS $$
BEGIN
	...
	DELETE FROM @extschema@.gogudb_config WHERE partrel = parent_relid;
	DELETE FROM @extschema@.gogudb_config_params WHERE partrel = parent_relid;
...
BEGIN
	...
	IF EXISTS (SELECT * FROM @extschema@.gogudb_config
...
/*
 * DDL trigger that removes entry from gogudb_config table.
 */
CREATE OR REPLACE FUNCTION @extschema@.gogudb_ddl_trigger_func()
...
BEGIN
	...
	JOIN @extschema@.gogudb_config AS cfg ON cfg.partrel::oid = events.objid
	WHERE events.classid = pg_class_oid AND events.objsubid = 0;

	/* Cleanup gogudb_config */
	DELETE FROM @extschema@.gogudb_config WHERE partrel = ANY(relids);

	/* Cleanup params table too */
	DELETE FROM @extschema@.gogudb_config_params WHERE partrel = ANY(relids);
END
$$ LANGUAGE plpgsql;
...
BEGIN
	...
	IF NOT EXISTS (SELECT FROM @extschema@.gogudb_config
				   WHERE partrel = parent_relid) THEN
...
END
$$ LANGUAGE plpgsql
SET gogudb.enable_partitionfilter = off;
 ...
 */
CREATE OR REPLACE FUNCTION @extschema@.gogudb_update_trigger_func()
RETURNS TRIGGER AS 'MODULE_PATHNAME', 'gogudb_update_trigger_func'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.create_update_triggers(
	parent_relid	REGCLASS)
RETURNS VOID AS 'MODULE_PATHNAME', 'create_update_triggers'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.create_single_update_trigger(
	parent_relid	REGCLASS,
	partition_relid	REGCLASS)
RETURNS VOID AS 'MODULE_PATHNAME', 'create_single_update_trigger'
LANGUAGE C STRICT;

/*
 * Check if relation has gogudb's UPDATE trigger.
 */
CREATE OR REPLACE FUNCTION @extschema@.has_update_trigger(
	parent_relid	REGCLASS)
RETURNS BOOL AS 'MODULE_PATHNAME', 'has_update_trigger'
LANGUAGE C STRICT;


/*
 * Create DDL trigger to call gogudb_ddl_trigger_func().
 */
CREATE EVENT TRIGGER gogudb_ddl_trigger
ON sql_drop
EXECUTE PROCEDURE @extschema@.gogudb_ddl_trigger_func();
...
RETURNS TEXT AS
$$
	SELECT expr FROM @extschema@.gogudb_config WHERE partrel = relid;
$$
LANGUAGE sql STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.get_partition_key_type(
	relid	REGCLASS)
RETURNS REGTYPE AS 'MODULE_PATHNAME', 'get_partition_key_type'
LANGUAGE C STRICT;
...
RETURNS INT4 AS
$$
	SELECT parttype FROM @extschema@.gogudb_config WHERE partrel = relid;
$$
LANGUAGE sql STRICT;


/*
 * Get number of partitions managed by gogudb.
 */
CREATE OR REPLACE FUNCTION @extschema@.get_number_of_partitions(
	parent_relid		REGCLASS)
RETURNS INT4 AS 'MODULE_PATHNAME', 'get_number_of_partitions_pl'
LANGUAGE C STRICT;

/*
 * Get parent of gogudb's partition.
 */
CREATE OR REPLACE FUNCTION @extschema@.get_parent_of_partition(
	partition_relid		REGCLASS)
RETURNS REGCLASS AS 'MODULE_PATHNAME', 'get_parent_of_partition_pl'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.get_base_type(
	typid	REGTYPE)
RETURNS REGTYPE AS 'MODULE_PATHNAME', 'get_base_type_pl'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.get_tablespace(
	relid	REGCLASS)
RETURNS TEXT AS 'MODULE_PATHNAME', 'get_tablespace_pl'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.validate_relname(
	relid	REGCLASS)
RETURNS VOID AS 'MODULE_PATHNAME', 'validate_relname'
LANGUAGE C;
...
CREATE OR REPLACE FUNCTION @extschema@.validate_expression(
	relid	REGCLASS,
	expression TEXT)
RETURNS VOID AS 'MODULE_PATHNAME', 'validate_expression'
LANGUAGE C;
...
CREATE OR REPLACE FUNCTION @extschema@.is_date_type(
	typid	REGTYPE)
RETURNS BOOLEAN AS 'MODULE_PATHNAME', 'is_date_type'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.is_operator_supported(
	type_oid	REGTYPE,
	opname		TEXT)
RETURNS BOOLEAN AS 'MODULE_PATHNAME', 'is_operator_supported'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.is_tuple_convertible(
	relation1	REGCLASS,
	relation2	REGCLASS)
RETURNS BOOL AS 'MODULE_PATHNAME', 'is_tuple_convertible'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.build_check_constraint_name(
	partition_relid	REGCLASS)
RETURNS TEXT AS 'MODULE_PATHNAME', 'build_check_constraint_name'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.build_update_trigger_name(
	relid			REGCLASS)
RETURNS TEXT AS 'MODULE_PATHNAME', 'build_update_trigger_name'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.build_update_trigger_func_name(
	relid			REGCLASS)
RETURNS TEXT AS 'MODULE_PATHNAME', 'build_update_trigger_func_name'
LANGUAGE C STRICT;


/*
 * Add record to gogudb_config (RANGE) and validate partitions.
 */
CREATE OR REPLACE FUNCTION @extschema@.add_to_gogudb_config(
	parent_relid		REGCLASS,
	expression			TEXT,
	range_interval		TEXT)
RETURNS BOOLEAN AS 'MODULE_PATHNAME', 'add_to_gogudb_config'
LANGUAGE C;

/*
 * Add record to gogudb_config (HASH) and validate partitions.
 */
CREATE OR REPLACE FUNCTION @extschema@.add_to_gogudb_config(
	parent_relid		REGCLASS,
	expression			TEXT)
...
CREATE OR REPLACE FUNCTION @extschema@.prevent_part_modification(
	parent_relid	REGCLASS)
RETURNS VOID AS 'MODULE_PATHNAME', 'prevent_part_modification'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.prevent_data_modification(
	parent_relid	REGCLASS)
RETURNS VOID AS 'MODULE_PATHNAME', 'prevent_data_modification'
LANGUAGE C STRICT;
...
RETURNS VOID AS 'MODULE_PATHNAME', 'invoke_on_partition_created_callback'
LANGUAGE C;
...
RETURNS VOID AS 'MODULE_PATHNAME', 'debug_capture'
LANGUAGE C STRICT;

CREATE OR REPLACE FUNCTION @extschema@.get_gogudb_lib_version()
RETURNS CSTRING AS 'MODULE_PATHNAME', 'get_gogudb_lib_version'
LANGUAGE C STRICT;

CREATE FUNCTION gogudb_fdw_handler()
RETURNS fdw_handler
AS 'MODULE_PATHNAME'
LANGUAGE C STRICT;

CREATE FUNCTION gogudb_fdw_validator(text[], oid)
RETURNS void
AS 'MODULE_PATHNAME'
LANGUAGE C STRICT;

CREATE FOREIGN DATA WRAPPER gogudb_fdw
  HANDLER gogudb_fdw_handler
  VALIDATOR gogudb_fdw_validator;
