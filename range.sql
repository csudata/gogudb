...
/*
 * Creates remote RANGE partitions for specified relation based on numeric attribute
 */
CREATE OR REPLACE FUNCTION @extschema@.create_remote_range_partitions(
	parent_relid	REGCLASS,
	remote_schema		TEXT,
	expression		TEXT,
	start_value		ANYELEMENT,
...
BEGIN
	...
	IF p_count != 0 THEN
		part_count := @extschema@.create_remote_range_partitions_internal(
									parent_relid,
...
/*
 * Creates remote RANGE partitions for specified relation based on datetime attribute
 */
CREATE OR REPLACE FUNCTION @extschema@.create_remote_range_partitions(
	parent_relid	REGCLASS,
	remote_schema		TEXT,
...
BEGIN
	...
	/* Insert new entry to gogudb config */
	PERFORM @extschema@.add_to_gogudb_config(parent_relid, expression, NULL);
...
CREATE OR REPLACE FUNCTION @extschema@.merge_range_partitions(
	partitions		REGCLASS[])
RETURNS VOID AS 'MODULE_PATHNAME', 'merge_range_partitions'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.drop_range_partition_expand_next(
	partition_relid		REGCLASS)
RETURNS VOID AS 'MODULE_PATHNAME', 'drop_range_partition_expand_next'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.create_range_partitions_internal(
	...
	tablespaces		TEXT[])
RETURNS REGCLASS AS 'MODULE_PATHNAME', 'create_range_partitions_internal'
LANGUAGE C;

CREATE OR REPLACE FUNCTION @extschema@.create_remote_range_partitions_internal(
	parent_relid	REGCLASS,
	bounds			ANYARRAY,
	partition_names	TEXT[],
	tablespaces		TEXT[],
	server_list		TEXT[],
	remote_schema		TEXT)
RETURNS REGCLASS AS 'MODULE_PATHNAME', 'create_remote_range_partitions_internal'
LANGUAGE C;
...
CREATE OR REPLACE FUNCTION @extschema@.create_single_range_partition(
	...
	partition_name	TEXT DEFAULT NULL,
	tablespace		TEXT DEFAULT NULL)
RETURNS REGCLASS AS 'MODULE_PATHNAME', 'create_single_range_partition_pl'
LANGUAGE C
...
CREATE OR REPLACE FUNCTION @extschema@.build_range_condition(
	...
	start_value		ANYELEMENT,
	end_value		ANYELEMENT)
RETURNS TEXT AS 'MODULE_PATHNAME', 'build_range_condition'
LANGUAGE C;

CREATE OR REPLACE FUNCTION @extschema@.build_sequence_name(
	parent_relid	REGCLASS)
RETURNS TEXT AS 'MODULE_PATHNAME', 'build_sequence_name'
LANGUAGE C STRICT;
...
CREATE OR REPLACE FUNCTION @extschema@.get_part_range(
	...
	dummy			ANYELEMENT)
RETURNS ANYARRAY AS 'MODULE_PATHNAME', 'get_part_range_by_idx'
LANGUAGE C;
...
CREATE OR REPLACE FUNCTION @extschema@.get_part_range(
	partition_relid	REGCLASS,
	dummy			ANYELEMENT)
RETURNS ANYARRAY AS 'MODULE_PATHNAME', 'get_part_range_by_oid'
LANGUAGE C;
...
RETURNS VOID AS 'MODULE_PATHNAME', 'check_range_available_pl'
LANGUAGE C;
...
CREATE OR REPLACE FUNCTION @extschema@.generate_range_bounds(
	...
	p_interval		INTERVAL,
	p_count			INTEGER)
RETURNS ANYARRAY AS 'MODULE_PATHNAME', 'generate_range_bounds_pl'
LANGUAGE C STRICT;
