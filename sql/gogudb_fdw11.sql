...
CREATE SERVER testserver1 FOREIGN DATA WRAPPER gogudb_fdw;
DO $d$
    BEGIN
        EXECUTE $$CREATE SERVER loopback FOREIGN DATA WRAPPER gogudb_fdw
            OPTIONS (dbname '$$||current_database()||$$',
                     port '$$||current_setting('port')||$$'
            )$$;
        EXECUTE $$CREATE SERVER loopback2 FOREIGN DATA WRAPPER gogudb_fdw
            OPTIONS (dbname '$$||current_database()||$$',
                     port '$$||current_setting('port')||$$'
            )$$;
    END;
...
CREATE FUNCTION gogudb_abs(int) RETURNS int AS $$
BEGIN
RETURN abs($1);
END
...
EXPLAIN (VERBOSE, COSTS OFF)
  SELECT count(c3) FROM ft1 t1 WHERE t1.c1 = gogudb_abs(t1.c2);
SELECT count(c3) FROM ft1 t1 WHERE t1.c1 = gogudb_abs(t1.c2);
EXPLAIN (VERBOSE, COSTS OFF)
  SELECT count(c3) FROM ft1 t1 WHERE t1.c1 === t1.c2;
SELECT count(c3) FROM ft1 t1 WHERE t1.c1 === t1.c2;
...
ALTER EXTENSION gogudb ADD FUNCTION gogudb_abs(int);
ALTER EXTENSION gogudb ADD OPERATOR === (int, int);
ALTER SERVER loopback OPTIONS (ADD extensions 'gogudb');
...
EXPLAIN (VERBOSE, COSTS OFF)
SELECT t1.c1, t2.c2, t1.c3 FROM ft1 t1 FULL JOIN ft2 t2 ON (t1.c1 = t2.c1) WHERE gogudb_abs(t1.c1) > 0 OFFSET 10 LIMIT 10;
...
alter extension gogudb add function least_accum(anyelement, variadic anyarray);
alter extension gogudb add aggregate least_agg(variadic items anyarray);
alter server loopback options (set extensions 'gogudb');
...
alter extension gogudb drop function least_accum(anyelement, variadic anyarray);
alter extension gogudb drop aggregate least_agg(variadic items anyarray);
alter server loopback options (set extensions 'gogudb');
...
-- Add into extension
alter extension gogudb add operator class my_op_class using btree;
alter extension gogudb add function my_op_cmp(a int, b int);
alter extension gogudb add operator family my_op_family using btree;
alter extension gogudb add operator public.<^(int, int);
alter extension gogudb add operator public.=^(int, int);
alter extension gogudb add operator public.>^(int, int);
alter server loopback options (set extensions 'gogudb');
...
alter extension gogudb drop operator class my_op_class using btree;
alter extension gogudb drop function my_op_cmp(a int, b int);
alter extension gogudb drop operator family my_op_family using btree;
alter extension gogudb drop operator public.<^(int, int);
alter extension gogudb drop operator public.=^(int, int);
alter extension gogudb drop operator public.>^(int, int);
alter server loopback options (set extensions 'gogudb');
...
EXPLAIN (verbose, costs off)
UPDATE ft2 SET c3 = 'bar' WHERE gogudb_abs(c1) > 2000 RETURNING *;            -- can't be pushed down
UPDATE ft2 SET c3 = 'bar' WHERE gogudb_abs(c1) > 2000 RETURNING *;
EXPLAIN (verbose, costs off)
...
CREATE SERVER fetch101 FOREIGN DATA WRAPPER gogudb_fdw OPTIONS( fetch_size '101' );
