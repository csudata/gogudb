...
static bool  trampoline_setup = false;

static void *planner_target_function = (void*)pg_plan_queries;
static List* (*trampoline_pg_plan_queries)(List *querytrees, 
						int cursorOptions, 
						ParamListInfo boundParams) = NULL;
static unsigned int trampoline_pg_plan_queries_size = 0;
static List* pg_plan_queries_patch(List *querytrees,
				   int cursorOptions, 
				   ParamListInfo boundParams);

static void *portalStart_target_function = (void*)PortalStart;
static void (*trampoline_portalStart)(Portal portal,
						ParamListInfo params,
						int eflags,
						Snapshot snapshot) = NULL;
static unsigned int trampoline_portStart_size = 0;
static void PortalStart_patch(Portal portal, ParamListInfo params, 
				int eflags, Snapshot snapshot);

static void *portalRun_target_function = (void*)PortalRun;
static bool (*trampoline_portalRun)(Portal portal, long count,
									bool isTopLevel, 
...
static unsigned int trampoline_portRun_size = 0;
static bool PortalRun_patch(Portal portal, long count, bool isTopLevel,
#if PG_VERSION_NUM >= 100000
							bool run_once,
#endif
							DestReceiver *dest,
...
static void replace_target(void);
...
static int do_hot_patch(void **trampoline_func_pp,
...
static void trampoline_dummy_func_for_mem()
{
	TRAMPOLINE_NOP_DEF
}
...
_PG_init(void)
{
	...
	{
		elog(ERROR, "gogudb module must be initialized by Postmaster. "
...
/* Get cached PATHMAN_CONFIG_PARAMS relation Oid */
Oid
get_table_partition_rule_relid(bool invalid_is_ok)
{
	/* Raise ERROR if Oid is invalid */
	if (!OidIsValid(pathman_config_relid) && !invalid_is_ok)
		elog(ERROR,
			 (!IsPathmanInitialized() ?
				"gogudb is not initialized yet" :
				"unexpected error in function "
						  CppAsString(get_table_partition_rule_relid)));

	return table_partition_rule_relid;
}

/* Get cached SERVER_MAP relation Oid */
Oid
get_server_map_relid(bool invalid_is_ok)
{
	/* Raise ERROR if Oid is invalid */
	if (!OidIsValid(server_map_relid) && !invalid_is_ok)
		elog(ERROR,
			 (!IsPathmanInitialized() ?
				"gogudb is not initialized yet" :
				"unexpected error in function "
						  CppAsString(get_server_map_relid)));

	return server_map_relid;
}
		...
		case PT_HASH:
			{
				...
				fill_type_cmp_fmgr_info(&cmp_finfo, getBaseType(INT4OID), getBaseType(INT4OID));

				/* Calculate 32-bit hash of 'value' and corresponding index */
				hash = OidFunctionCall1(prel->hash_proc, value);
				idx = hash_to_part_index(DatumGetInt32(hash), HASH_SLOT_SIZE);
				select_range_partitions(idx,
										collid,
										&cmp_finfo,
						...
						const char *target_name)
{
	...
	*trampoline_func_pp = (void*)(addrs & ~0x0F);
	// hot patch functions
	res = hot_patch_function(target_function, patch_function, *trampoline_func_pp,
							trampoline_size, &used_size, 1);
	if (res != 0) {
		elog(ERROR, "unable to hot patch %s (%p). res: %d.",
			 target_name, target_function, res);
                return 1;
	}

	elog(DEBUG3, "hot patch for: %s (%p) complete. Patch func: %p, "
		 "Trampoline address: %p, size: %u, used: %u.",
		 target_name, target_function, patch_function, 
		 *trampoline_func_pp, *trampoline_size, used_size);
...
static void replace_target(void) 
{
	...
	size_t func_in_pg = 0;
	size_t func_in_plugin = 0;
	int mmap_flags = MAP_PRIVATE|MAP_ANONYMOUS;
	trampoline_mem = NULL;

#ifdef __x86_64__
	func_in_pg = (size_t)pg_plan_queries;
	func_in_plugin = (size_t)trampoline_dummy_func_for_mem;
	if (func_in_pg < INT_MAX && func_in_plugin > INT_MAX) {
		trampoline_mem = mmap(NULL, page_size, PROT_READ|PROT_EXEC, mmap_flags| MAP_32BIT , -1, 0);
		if (MAP_FAILED == trampoline_mem) {
			trampoline_mem = NULL;
			elog(DEBUG3, "unable to mmap 32bit memory size: %lu, errno: %d. "
				"This may happen when 32bit address space is used up. "
				"Will try static and regular mmap...", page_size, errno);
		} else {
			elog(DEBUG3, "mem via 32bit mmap: %p page size: %ld", trampoline_mem, page_size);
		}
	}
#endif
	...
	if (!trampoline_mem) {
		if (use_static_memory) {
			...
			elog(DEBUG3, "mem func addr: %p mem start addr: %p page size: %ld",
				 trampoline_dummy_func_for_mem, trampoline_mem, page_size);
		} else {
			trampoline_mem = mmap(NULL, page_size, PROT_READ|PROT_EXEC, mmap_flags , -1, 0);
			if (MAP_FAILED == trampoline_mem) {
				trampoline_mem = NULL;
				elog(WARNING, "unable to mmap 32bit memory size: %lu, errno: %d. Abborting", page_size, errno);
				return;
			} else {
				elog(DEBUG3, "mem via mmap: %p page size: %ld", trampoline_mem, page_size);
			}
		}
	}
	...
	if (do_hot_patch((void **)&trampoline_pg_plan_queries, &trampoline_pg_plan_queries_size,
                        planner_target_function, (void *)pg_plan_queries_patch, "pg_plan_queries"))
	{

   		elog(ERROR, "Failed to inject pg_plan_queries");
		return ;
	}

	if (do_hot_patch((void **)&trampoline_portalRun, &trampoline_portRun_size,
                        portalRun_target_function, (void *)PortalRun_patch, "PortalRun"))
	{

   		elog(ERROR, "Failed to inject PortalRun");
		return ;
	}

	if (do_hot_patch((void **)&trampoline_portalStart, &trampoline_portStart_size,
                        portalStart_target_function, (void *)PortalStart_patch, "PortalStart"))
	{

   		elog(ERROR, "Failed to inject PortalStart");
		return ;
	}
	trampoline_setup = true;
}

__attribute__ ((noinline))
static List* pg_plan_queries_patch(List *querytrees, int cursorOptions, 
									ParamListInfo boundParams)
{
 	List *result = pg_plan_queries_hook(querytrees, cursorOptions, boundParams);
	if (result != NIL)
		return result;
	else 
		return trampoline_pg_plan_queries(querytrees, cursorOptions, boundParams);
}
...
static void PortalStart_patch(Portal portal, ParamListInfo params, 
								int eflags, Snapshot snapshot)
{
	if (!PortalStart_hook(portal, params, eflags, snapshot))
		trampoline_portalStart(portal, params, eflags, snapshot);
	return;
}
...
static bool PortalRun_patch(Portal portal, long count, bool isTopLevel,
							...
							DestReceiver *altdest, char *completionTag)
{
	if (PortalRun_hook(portal, count, isTopLevel,
#if PG_VERSION_NUM >= 100000
						run_once, 
#endif
		...
		return trampoline_portalRun(portal, count, isTopLevel,
#if PG_VERSION_NUM >= 100000
									run_once, 
#endif
									dest, altdest, completionTag);
