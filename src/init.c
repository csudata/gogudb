...
MemoryContext		RangeServerSetContext			= NULL;
...
RangeServerSet			*rangeServerSet = NULL;
...
static bool validate_hash_range_opexpr(const Expr *expr,
						 const PartRelationInfo *prel,
...
static int rangeserver_comparator(const void *a, const void *b);
						   ...
						   bool *found)
{
	...
	if (cache_table == NULL)
		switch (action)
		{
			...
			case HASH_ENTER:
			case HASH_REMOVE:
				elog(ERROR, "gogudb is not initialized yet");
				break;
...
load_config(void)
{
	...
	/* Read SERVER_MAP table */
	(void)read_range_server_set();
	...
	elog(DEBUG2, "gogudb's config has been loaded successfully [%u]", MyProcPid);
...
init_pathman_relation_oids(void)
{
	...
	/* Cache partition_table_rule relation's Oid */
	table_partition_rule_relid = get_relname_relid(TABLE_PARTITION_RULE,
													schema);
	if (table_partition_rule_relid == InvalidOid)
		return false;

	/* Cache server_map relation's Oid */
	server_map_relid = get_relname_relid(SERVER_MAP, schema);
	if (server_map_relid == InvalidOid)
		return false;
...
fini_pathman_relation_oids(void)
{
	...
	table_partition_rule_relid = InvalidOid;
	server_map_relid = InvalidOid;
...
init_local_cache(void)
{
	...
	if (TopPathmanContext)
	{
		...
		Assert(MemoryContextIsValid(RangeServerSetContext));
	...
	else
	{
		...
		Assert(RangeServerSetContext == NULL);
		...
		/* For RangeServerSet */
		RangeServerSetContext =
				AllocSetContextCreate(TopPathmanContext,
									  CppAsString(RangeServerSetContext),
									  ALLOCSET_DEFAULT_SIZES);
	}
	...
	partitioned_rels = hash_create("gogudb's partition dispatch cache",
								   PART_RELS_SIZE, &ctl,
	...
	parent_cache = hash_create("gogudb's partition parents cache",
	...
	bound_cache = hash_create("gogudb's partition bounds cache",
							  PART_RELS_SIZE * CHILD_FACTOR, &ctl,
							  HASH_ELEM | HASH_BLOBS | HASH_CONTEXT);
}
...
build_check_constraint_name_relname_internal(const char *relname)
{
	...
	return psprintf("gogudb_%s_check", relname);
}
								 ...
								 TransactionId *xmin, ItemPointerData* iptr)
{
	...
	elog(DEBUG2, "TABLE_CONFIG table %s relation %u",
...
/*
 * Loads table partition parameters
 * or 'auto' from PARTITION_TABLE_RULE.
 */
bool
read_table_partition_rule_params(const char* schema, const char* table, Datum *values, bool *isnull)
{
	Relation		rel;
	HeapScanDesc	scan;
	ScanKeyData		key[2];
	...
	ScanKeyInit(&key[0],
				Anum_table_partition_rule_schema,
				BTEqualStrategyNumber, F_TEXTEQ,
				CStringGetTextDatum(schema));

	ScanKeyInit(&key[1],
				Anum_table_partition_rule_relname,
				BTEqualStrategyNumber, F_TEXTEQ,
				CStringGetTextDatum(table));

	rel = heap_open(get_table_partition_rule_relid(false), AccessShareLock);
	snapshot = RegisterSnapshot(GetLatestSnapshot());
	scan = heap_beginscan(rel, snapshot, 2, key);
	...
	if ((htup = heap_getnext(scan, ForwardScanDirection)) != NULL)
	{
		...
		if (values != NULL && isnull != NULL) {
			...
			/* Perform checks for non-NULL columns */
			Assert(!isnull[Anum_table_partition_rule_relname - 1]);
			Assert(!isnull[Anum_table_partition_rule_schema - 1]);
			Assert(!isnull[Anum_table_partition_rule_cooked_expr - 1]);
			Assert(!isnull[Anum_table_partition_rule_parttype - 1]);
			Assert(!isnull[Anum_table_partition_rule_patitions_dist - 1]);
		}
		row_found = true;
	}
...
/*
 * check whether schema name is used in table partition rules
 * */
bool
schema_in_table_partition_rule(const char* schema)
{
	Relation		rel;
	HeapScanDesc	scan;
	ScanKeyData		key[1];
...
static int
rangeserver_comparator(const void *a, const void *b)
{
        if (((const RangeServer *) a)->hash_range_start == ((const RangeServer *) b)->hash_range_start)
                return 0;
        else
                return (((const RangeServer *) b)->hash_range_start < ((const RangeServer *) a)->hash_range_start) ? 1 : -1;
}

/*
 * Go through the SERVER_MAP table and create  entries.
 */
bool 
read_range_server_set(void)
{
	Relation		rel;
	HeapScanDesc	scan;
	Snapshot		snapshot;
	...
	int			count = 0;
	bool			result = true;
	RangeServer 		tmpRSS[HASH_SLOT_SIZE];
	MemoryContext		oldcontext;

	oldcontext = MemoryContextSwitchTo(RangeServerSetContext);
	/* Open SERVER_MAP with latest snapshot available */
	rel = heap_open(get_server_map_relid(false), AccessShareLock);

	/* Check that number of columns == Natts_pathman_config */
	Assert(RelationGetDescr(rel)->natts == Natts_server_map);
	...
	/* Examine each row and create a PartRelationInfo in local cache */
	while(((htup = heap_getnext(scan, ForwardScanDirection)) != NULL)
		&& (count < HASH_SLOT_SIZE))
	{
		Datum		values[Natts_server_map];
		bool		isnull[Natts_server_map];

		/* Extract Datums from tuple 'htup' */
		heap_deform_tuple(htup, RelationGetDescr(rel), values, isnull);

		/* These attributes are marked as NOT NULL, check anyway */
		Assert(!isnull[Anum_server_map_svrname - 1]);
		Assert(!isnull[Anum_server_map_range_start - 1]);
		Assert(!isnull[Anum_server_map_range_end - 1]);

		tmpRSS[count].server_name = pstrdup(TextDatumGetCString(values[Anum_server_map_srvname - 1]));
		tmpRSS[count].hash_range_start = DatumGetInt16(values[Anum_server_map_range_start - 1]);
		tmpRSS[count].hash_range_end = DatumGetInt16(values[Anum_server_map_range_end - 1]);
		Assert((tmpRSS[count].hash_start <= 127) && (tmpRSS[count].hash_start >=0) && 
			(tmpRSS[count].hash_start < tmpRSS[count].hash_end) && (tmpRSS[count].hash_end <= 127));
		count++;
	}
	/* Clean resources */
	heap_endscan(scan);
	...
	if (count > 0) {
		int i = 0;
		qsort(tmpRSS, count, sizeof(RangeServer), rangeserver_comparator);
		if (tmpRSS[0].hash_range_start != 0)
			elog(ERROR, "Fisrt hash range %s:[%d  %d) should begin with 0 ",
				tmpRSS[i].server_name, tmpRSS[i].hash_range_start,
				tmpRSS[i].hash_range_end);

		for(; i < count-1; i++) {

			if (tmpRSS[i].hash_range_end != tmpRSS[i+1].hash_range_start) {
				elog(WARNING, "Found hash range overlap or hole between %s:[%d  %d) and %s:[%d %d) ",
					tmpRSS[i].server_name, tmpRSS[i].hash_range_start,
					tmpRSS[i].hash_range_end, tmpRSS[i+1].server_name,
					tmpRSS[i+1].hash_range_start,tmpRSS[i+1].hash_range_end);
				result = false;
				goto read_server_end;
			}

		}

		if (tmpRSS[count -1 ].hash_range_end != HASH_SLOT_SIZE) {
			elog(WARNING, "last hash range of %s:[%d %d) should be end with %d",
				tmpRSS[count -1].server_name, 
				tmpRSS[count -1].hash_range_end,
				tmpRSS[count -1].hash_range_start,
				HASH_SLOT_SIZE);
			result = false;
			goto read_server_end;
		}

		rangeServerSet = palloc(sizeof(int)+sizeof(RangeServer)*count);
		rangeServerSet->server_count = count;
		memcpy(rangeServerSet->server_set, tmpRSS, sizeof(RangeServer)*count);
	}
 ...
 *
 *		get_hash_part_idx(TYPE_HASH_PROC(VALUE), HASH_SLOT_SIZE) >= left_const
 *		AND get_hash_part_idx(TYPE_HASH_PROC(VALUE), HASH_SLOT_SIZE) < right_const
 *
 * Writes 'part_idx' hash value for this partition on success.
 */
bool
validate_hash_range_constraint(const Expr *expr,
						  const PartRelationInfo *prel,
						  uint32 *lower, uint32 *upper)
{
	...
	if (and_clause((Node *) expr))
	{
		...
		foreach (lc, boolexpr->args)
		{
			...
			/* Exit immediately if something is wrong */
			if (!validate_hash_range_opexpr((const Expr *) opexpr, prel, 
							lower, upper, &lower_null, &upper_null))
				return false;
		}
...
static bool
validate_hash_range_opexpr(const Expr *expr,
						 const PartRelationInfo *prel,
						 uint32 *lower, uint32 *upper,
						 bool *lower_null, bool *upper_null)
{
	...
	op_expr = (const OpExpr *) expr;

	/* Check that left expression is a function call */
	if (!IsA(linitial(op_expr->args), FuncExpr))
		return false;

	get_hash_expr = (FuncExpr *) linitial(op_expr->args); /* get_hash_part_idx(...) */

	tce = lookup_type_cache(get_hash_expr->funcresulttype, TYPECACHE_BTREE_OPFAMILY);
	/* Is 'eqexpr' an equality operator? */

	strategyNumber =  get_op_opfamily_strategy(op_expr->opno, tce->btree_opf);
	if (strategyNumber == BTGreaterEqualStrategyNumber)
	{
		/* Bound already exists */
		if (*lower_null == false)
			return false;
		...
		*lower_null = false;
	} else if  (strategyNumber == BTLessStrategyNumber)
	{
		/* Bound already exists */
		if (*upper_null == false)
			return false;
	...
	if (list_length(get_hash_expr->args) == 2)
	{
		...
		Node   *second = lsecond(get_hash_expr->args);	/* arg #2: HASH_SLOT_SIZE */
		Const  *side_limit;						/* size limit for this partition */
		...
		if (DatumGetUInt32(((Const *) second)->constvalue) != HASH_SLOT_SIZE)
			return false;

		/* Check that side limit is Const */
		if (!IsA(lsecond(op_expr->args), Const))
			return false;

		/* Fetch side limit */
		side_limit = lsecond(op_expr->args);

		/* Check that side limit is NOT NULL */
		if (side_limit->constisnull)
			return false;

		if (strategyNumber == BTGreaterEqualStrategyNumber)
		{

			*lower = DatumGetUInt32(side_limit->constvalue);
			if (*lower < 0)
				return false;
		}
		else {
			*upper = DatumGetUInt32(side_limit->constvalue);
			if (*upper > HASH_SLOT_SIZE)
				return false;
		}
...
validate_sql_facade_version(uint32 ver)
{
	...
	{
		elog(INFO, "current version: %x, lowest compatible: %x",
					 ver, LOWEST_COMPATIBLE_FRONT);

		DisablePathman(); /* disable pg_pathman since config is broken */
		ereport(ERROR,
				(errmsg("gogudb's Pl/PgSQL frontend is incompatible with "
						"its shared library"),
