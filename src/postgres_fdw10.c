...
//PG_MODULE_MAGIC; remove it since we decleared it in pg_pathma.c already
...
/* If fetch remote tuples more than it , use cursor to fetch data */
#define USE_CUROSR_THRESHOLD		5000

/*
 *  * 记录扫描每个远程的foreign server的次数，如果次数大于1，则需要走原先的游标，否则就可以不用游标，以提升性能。
 *   */
typedef Oid RemoteCacheKey;
static HTAB * RemoteHash = NULL;
 ...
 * SQL functions
 */
PG_FUNCTION_INFO_V1(gogudb_fdw_handler);
...
static void fetch_more_data_without_cursor(ForeignScanState *node);
...
static RemoteCacheEntry *GetRemoteEntry(UserMapping *user);
/*
 * Foreign-data wrapper handler function: return a struct with pointers
 * to my callback routines.
 */
Datum
gogudb_fdw_handler(PG_FUNCTION_ARGS)
{
						  ...
						  Oid foreigntableid)
{
	 ...
	 */
	GoguclassifyConditions(root, baserel, baserel->baserestrictinfo,
...
get_useful_pathkeys_for_relation(PlannerInfo *root, RelOptInfo *rel)
{
	...
	if (root->query_pathkeys)
	{
		...
		foreach(lc, root->query_pathkeys)
		{
			 ...
			 *
			 * Gogu_is_foreign_expr would detect volatile expressions as well, but
			 * checking ec_has_volatile here saves some cycles.
			 */
			if (pathkey_ec->ec_has_volatile ||
				!(em_expr = Gogu_find_em_expr_for_rel(pathkey_ec, rel)) ||
				!Gogu_is_foreign_expr(root, rel, em_expr))
			{
	...
	foreach(lc, useful_eclass_list)
	{
		...
		em_expr = Gogu_find_em_expr_for_rel(cur_ec, rel);
		if (em_expr == NULL || !Gogu_is_foreign_expr(root, rel, em_expr))
			continue;
						...
						Oid foreigntableid)
{
	...
	foreach(lc, baserel->joininfo)
	{
		...
		if (!Gogu_is_foreign_expr(root, baserel, rinfo->clause))
			continue;
 ...
 *   *		获得某个远程相对应的数据
 *    */
static RemoteCacheEntry *
GetRemoteEntry(UserMapping *user)
{
	bool		found;
	RemoteCacheEntry *entry;
	RemoteCacheKey key;
    
	/* First time through, initialize Remote cache hashtable */
	if (RemoteHash == NULL)
	{
		HASHCTL		ctl;
        
		MemSet(&ctl, 0, sizeof(ctl));
		ctl.keysize = sizeof(RemoteCacheKey);
		ctl.entrysize = sizeof(RemoteCacheEntry);
		/* allocate ConnectionHash in the cache context */
		ctl.hcxt = CacheMemoryContext;
		RemoteHash = hash_create("gogu_fdw remote scan count", 8, &ctl,
	...
	/*
	 ** Find or create cached entry for remote.
	 **/
	entry = hash_search(RemoteHash, &key, HASH_ENTER, &found);
	if (!found)
	{
        	entry->scan_count = 0;
    	}
					   ...
					   Plan *outer_plan)
{
	...
	if (IS_SIMPLE_REL(foreignrel))
	{
		...
		foreach(lc, scan_clauses)
		{
			...
			else if (Gogu_is_foreign_expr(root, foreignrel, rinfo->clause))
				remote_exprs = lappend(remote_exprs, rinfo->clause);
			else
				local_exprs = lappend(local_exprs, rinfo->clause);
		}
	...
	else
	{
		...
		fdw_scan_tlist = Gogu_build_tlist_to_deparse(foreignrel);
	...
	user_mapping = GetUserMapping(foreignrel->userid ? foreignrel->userid 
								: GetUserId(),
					foreignrel->serverid);
	entry = GetRemoteEntry(user_mapping);
	entry->scan_count++;
	 ...
	 */
	initStringInfo(&sql);
	GogudeparseSelectStmtForRel(&sql, root, foreignrel, fdw_scan_tlist,
...
postgresBeginForeignScan(ForeignScanState *node, int eflags)
{
	...
	fsstate->server_count = GetRemoteEntry(user);
	
	/*
	 * Get connection to the foreign server.  Connection manager will
	 * establish new connection if necessary.
	 */
	if (fsstate->server_count->scan_count > 1 ||
		fsplan->scan.plan.plan_rows > USE_CUROSR_THRESHOLD) {
		/* Assign a unique ID for my cursor */
		fsstate->cursor_number = GoguGetCursorNumber(fsstate->conn);

		fsstate->conn = GoguGetConnection(user, false, true);
	} else 
		fsstate->conn = GoguGetConnection(user, false, false);
	...
	if (fsstate->cursor_number == 0) {
		/* since have no cursor numer, means does not use cursor */
		if (numParams > 0)             
		{
			MemoryContext oldcontext;      
			ExprContext *econtext = node->ss.ps.ps_ExprContext;
			oldcontext = MemoryContextSwitchTo(econtext->ecxt_per_tuple_memory);
			process_query_params(econtext, fsstate->param_flinfo,
						fsstate->param_exprs, fsstate->param_values);        
			MemoryContextSwitchTo(oldcontext);
                }

		if (!PQsendQueryParams(fsstate->conn, fsstate->query, numParams,
								NULL, fsstate->param_values, NULL, NULL, 0))
			Gogu_pgfdw_report_error(ERROR, NULL, fsstate->conn, false, fsstate->query);
		
		PQsetSingleRowMode(fsstate->conn);
	}
}
...
postgresIterateForeignScan(ForeignScanState *node)
{
	 ...
	 */
	if (fsstate->cursor_number > 0 && !fsstate->cursor_exists)
		create_cursor(node);
	...
	if (fsstate->next_tuple >= fsstate->num_tuples)
	{
		...
		if (!fsstate->eof_reached) {
			if (fsstate->cursor_number > 0)
				...
				fetch_more_data_without_cursor(node);
		}
...
postgresReScanForeignScan(ForeignScanState *node)
{
	...
	if (fsstate->cursor_number > 0) {
	 	...
	 	*/
		res = Gogu_pgfdw_exec_query(fsstate->conn, sql);
		if (PQresultStatus(res) != PGRES_COMMAND_OK)
			Gogu_pgfdw_report_error(ERROR, res, fsstate->conn, true, sql);
		PQclear(res);
	} else {
		PGconn	   *conn = fsstate->conn;
			...
			if (!fsstate->eof_reached) {
				while (true)
				{
					...
					if (PQresultStatus(res) != PGRES_SINGLE_TUPLE)
						Gogu_pgfdw_report_error(ERROR, res, fsstate->conn, false, fsstate->query);
				}
			}
			...
			if (!PQsendQueryParams(fsstate->conn, fsstate->query, fsstate->numParams,
								NULL, fsstate->param_values, NULL, NULL, 0))
				Gogu_pgfdw_report_error(ERROR, NULL, fsstate->conn, false, fsstate->query);
...
postgresEndForeignScan(ForeignScanState *node)
{
	...
	if (fsstate->cursor_exists) {
		...
		if (!fsstate->eof_reached && fsstate->cursor_number==0) {
			PGconn	   *conn = fsstate->conn;
			PGresult   *res;
	...
	GoguReleaseConnection(fsstate->conn);
	fsstate->conn = NULL;
	fsstate->server_count->scan_count--;
						  ...
						  int subplan_index)
{
	...
	{
		case CMD_INSERT:
			GogudeparseInsertSql(&sql, root, resultRelation, rel,
							 targetAttrs, doNothing, returningList,
							 &retrieved_attrs);
			break;
		case CMD_UPDATE:
			GogudeparseUpdateSql(&sql, root, resultRelation, rel,
							 targetAttrs, returningList,
							 &retrieved_attrs);
			break;
		case CMD_DELETE:
			GogudeparseDeleteSql(&sql, root, resultRelation, rel,
							 returningList,
							 &retrieved_attrs);
			break;
		default:
						   ...
						   int eflags)
{
	...
	fmstate->conn = GoguGetConnection(user, true, true);
						  ...
						  TupleTableSlot *planSlot)
{
	...
	if (!PQsendQueryPrepared(fmstate->conn,
							 ...
							 NULL,
							 0))
		Gogu_pgfdw_report_error(ERROR, NULL, fmstate->conn, false, fmstate->query);
	 ...
	 * without releasing the PGresult.
	 */
	res = Gogu_pgfdw_get_result(fmstate->conn, fmstate->query);
	if (PQresultStatus(res) !=
		(fmstate->has_returning ? PGRES_TUPLES_OK : PGRES_COMMAND_OK))
		Gogu_pgfdw_report_error(ERROR, res, fmstate->conn, true, fmstate->query);
						 ...
						 ResultRelInfo *resultRelInfo)
{
	...
	if (fmstate->p_name)
	{
		 ...
		 * without releasing the PGresult.
		 */
		res = Gogu_pgfdw_exec_query(fmstate->conn, sql);
		if (PQresultStatus(res) != PGRES_COMMAND_OK)
			Gogu_pgfdw_report_error(ERROR, res, fmstate->conn, true, sql);
		PQclear(res);
		fmstate->p_name = NULL;
	}

	/* Release remote connection */
	GoguReleaseConnection(fmstate->conn);
	fmstate->conn = NULL;
}
						 ...
						 int subplan_index)
{
	...
	if (operation == CMD_UPDATE)
	{
		...
		while ((col = bms_next_member(rte->updatedCols, col)) >= 0)
		{
			...
			if (!Gogu_is_foreign_expr(root, foreignrel, (Expr *) tle->expr))
				return false;
	...
	{
		case CMD_UPDATE:
			GogudeparseDirectUpdateSql(&sql, root, resultRelation, rel,
			...
			break;
		case CMD_DELETE:
			GogudeparseDirectDeleteSql(&sql, root, resultRelation, rel,
								   remote_exprs, &params_list,
								   returningList, &retrieved_attrs);
			break;
		default:
...
postgresBeginDirectModify(ForeignScanState *node, int eflags)
{
	 ...
	 * establish new connection if necessary.
	 */
	dmstate->conn = GoguGetConnection(user, false, true);
...
postgresEndDirectModify(ForeignScanState *node)
{
	...
	/* Release remote connection */
	GoguReleaseConnection(dmstate->conn);
	dmstate->conn = NULL;
						...
						Cost *p_startup_cost, Cost *p_total_cost)
{
	...
	if (fpinfo->use_remote_estimate)
	{
		 ...
		 */
		GoguclassifyConditions(root, foreignrel, param_join_conds,
		...
		conn = GoguGetConnection(fpinfo->user, false, true);
					...
					Cost *startup_cost, Cost *total_cost)
{
	...
	PG_TRY();
	{
		 ...
		 */
		res = Gogu_pgfdw_exec_query(conn, sql);
		if (PQresultStatus(res) != PGRES_TUPLES_OK)
			Gogu_pgfdw_report_error(ERROR, res, conn, false, sql);
...
create_cursor(ForeignScanState *node)
{
	...
	if (!PQsendQueryParams(conn, buf.data, numParams,
		...
		Gogu_pgfdw_report_error(ERROR, NULL, conn, false, buf.data);
	 ...
	 * without releasing the PGresult.
	 */
	res = Gogu_pgfdw_get_result(conn, buf.data);
	if (PQresultStatus(res) != PGRES_COMMAND_OK)
		Gogu_pgfdw_report_error(ERROR, res, conn, true, fsstate->query);
	PQclear(res);
...
/*
 * Fetch some more rows without cursor.
 */
static void
fetch_more_data_without_cursor(ForeignScanState *node)
{
	PgFdwScanState *fsstate = (PgFdwScanState *) node->fdw_state;
	PGresult   *volatile res = NULL;
	MemoryContext oldcontext;
	...
	PG_TRY();
	{
		...
		fsstate->tuples = (HeapTuple *) palloc0(fsstate->fetch_size * sizeof(HeapTuple));
		fsstate->next_tuple = 0;

		while (i < fsstate->fetch_size)
		{
		...
		fsstate->eof_reached = (i < fsstate->fetch_size);
		fsstate->num_tuples = i;
		PQclear(res);
		res = NULL;
	}
	PG_CATCH();
...
fetch_more_data(ForeignScanState *node)
{
	...
	PG_TRY();
	{
		...
		if (PQresultStatus(res) != PGRES_TUPLES_OK)
			Gogu_pgfdw_report_error(ERROR, res, conn, false, fsstate->query);
 ...
 */
int
Gogu_set_transmission_modes(void)
{
 ...
 */
void
Gogu_reset_transmission_modes(int nestlevel)
{
...
close_cursor(PGconn *conn, unsigned int cursor_number)
{
	...
	res = Gogu_pgfdw_exec_query(conn, sql);
	if (PQresultStatus(res) != PGRES_COMMAND_OK)
		Gogu_pgfdw_report_error(ERROR, res, conn, true, sql);
	PQclear(res);
}
...
prepare_foreign_modify(PgFdwModifyState *fmstate)
{
	...
	snprintf(prep_name, sizeof(prep_name), "pgsql_fdw_prep_%u",
			 GoguGetPrepStmtNumber(fmstate->conn));
						 ...
						 TupleTableSlot *slot)
{
	...
	if (slot != NULL && fmstate->target_attrs != NIL)
	{
		...
		nestlevel = Gogu_set_transmission_modes();
		...
		Gogu_reset_transmission_modes(nestlevel);
	}
...
execute_dml_stmt(ForeignScanState *node)
{
	...
	if (!PQsendQueryParams(dmstate->conn, dmstate->query, numParams,
						   NULL, values, NULL, NULL, 0))
		Gogu_pgfdw_report_error(ERROR, NULL, dmstate->conn, false, dmstate->query);
	 ...
	 * without releasing the PGresult.
	 */
	dmstate->result = Gogu_pgfdw_get_result(dmstate->conn, dmstate->query);
	if (PQresultStatus(dmstate->result) !=
		(dmstate->has_returning ? PGRES_TUPLES_OK : PGRES_COMMAND_OK))
		Gogu_pgfdw_report_error(ERROR, dmstate->result, dmstate->conn, true,
						   dmstate->query);
							...
							BlockNumber *totalpages)
{
	...
	*func = postgresAcquireSampleRowsFunc;
	...
	conn = GoguGetConnection(user, false, true);

	/*
	 * Construct command to get page count for relation.
	 */
	initStringInfo(&sql);
	GogudeparseAnalyzeSizeSql(&sql, relation);

	/* In what follows, do not risk leaking any PGresults. */
	PG_TRY();
	{
		res = Gogu_pgfdw_exec_query(conn, sql.data);
		if (PQresultStatus(res) != PGRES_TUPLES_OK)
			Gogu_pgfdw_report_error(ERROR, res, conn, false, sql.data);

		if (PQntuples(res) != 1 || PQnfields(res) != 1)
			elog(ERROR, "unexpected result from GogudeparseAnalyzeSizeSql query");
							  ...
							  double *totaldeadrows)
{
	 ...
	 */
	cursor_number = GoguGetCursorNumber(conn);
	initStringInfo(&sql);
	appendStringInfo(&sql, "DECLARE c%u CURSOR FOR ", cursor_number);
	GogudeparseAnalyzeSql(&sql, relation, &astate.retrieved_attrs);
	...
	PG_TRY();
	{
		...
		for (;;)
		{
			...
			res = Gogu_pgfdw_exec_query(conn, fetch_sql);
			/* On error, report the original query, not the FETCH. */
			if (PQresultStatus(res) != PGRES_TUPLES_OK)
				Gogu_pgfdw_report_error(ERROR, res, conn, false, sql.data);
...
postgresImportForeignSchema(ImportForeignSchemaStmt *stmt, Oid serverOid)
{
	...
	conn = GoguGetConnection(mapping, false, true);
	...
	PG_TRY();
	{
		...
		GogudeparseStringLiteral(&buf, stmt->remote_schema);

		res = Gogu_pgfdw_exec_query(conn, buf.data);
		if (PQresultStatus(res) != PGRES_TUPLES_OK)
			Gogu_pgfdw_report_error(ERROR, res, conn, false, buf.data);
			...
			stmt->list_type == FDW_IMPORT_SCHEMA_EXCEPT)
		{
			...
			foreach(lc, stmt->table_list)
			{
				...
				if (first_item)
					...
					appendStringInfoString(&buf, ", ");
				GogudeparseStringLiteral(&buf, rv->relname);
			}
		...
		for (i = 0; i < numrows;)
		{
			...
			do
			{
				...
				GogudeparseStringLiteral(&buf, attname);
				appendStringInfoChar(&buf, ')');
			...
			GogudeparseStringLiteral(&buf, tablename);
				...
				JoinPathExtraData *extra)
{
	...
	{
		RestrictInfo *rinfo = lfirst_node(RestrictInfo, lc);
		bool		is_remote_clause = Gogu_is_foreign_expr(root, joinrel,
													   rinfo->clause);
	...
	appendStringInfo(fpinfo->relation_name, "(%s) %s JOIN (%s)",
					 ...
					 Gogu_get_jointype_name(fpinfo->jointype),
	...
	foreach(lc, fpinfo->server->options)
	{
		...
		if (strcmp(def->defname, "use_remote_estimate") == 0)
			...
			fpinfo->shippable_extensions =
				GoguExtractExtensionList(defGetString(def), false);
...
foreign_grouping_ok(PlannerInfo *root, RelOptInfo *grouped_rel)
{
	...
	foreach(lc, grouping_target->exprs)
	{
		...
		if (sgref && get_sortgroupref_clause_noerr(sgref, query->groupClause))
		{
			 ...
			 */
			if (!Gogu_is_foreign_expr(root, grouped_rel, expr))
				return false;
		...
		else
		{
			...
			else
			{
				 ...
				 */
				if (!Gogu_is_foreign_expr(root, grouped_rel, (Expr *) aggvars))
					return false;
 ...
 */
extern Expr *
Gogu_find_em_expr_for_rel(EquivalenceClass *ec, RelOptInfo *rel)
{
	ListCell   *lc_em;
