...
static const char *log_prefix = "gogudb:";
...
static int protect(void *addr, size_t len)
{
	...
	if (res)
	{
		elog(DEBUG3, "%s unable to protect mode: PROT_READ|PROT_EXEC. "
			 "Page: %p, Size: %zu, errno: %d, res %d.",
			 log_prefix, (void *)addr, len, errno, res);
		// fail only if nx bit is enabled
		fp = fopen("/proc/cpuinfo", "r");
		if (NULL == fp)
		{
			elog(ERROR, "%s unable to verify nx bit. Failed checking /proc/cpuinfo. "
				 "This may happen if you have SELinux enabled. Disable SELinux "
				 "execmod protection for postgres. Page: %p, Size: %zu, errno: %d.",
		...
		while (fgets(buff, 1024, fp) != NULL)
		{
			...
			{
				nxchecked = true;
				elog(DEBUG3, "%s cpuinfo flags line: %s. ",log_prefix, line);
				if (strstr(line, " nx")) // nx enabled so fail
				{
					fclose(fp);
					elog(ERROR, "%s unable to protect page and nx bit enabled."
						" This may happen if you have SELinux enabled. Disable SELinux "
						"execmod protection for mysqld. Page: %p, Size: %zu.",
		...
		{
			elog(ERROR, "%s unable to verify nx bit. Failed finding: %s in "
				"/proc/cpuinfo. This may happen if you have SELinux enabled."
				" Disable SELinux execmod ""protection for mysqld. Page: %p, Size: %zu.",
...
static int unprotect(void *addr, size_t len)
{
	...
	if (use_exec_prot)
	{
		...
		if (res)
		{
			...
			if (res)
			{
				elog(ERROR, "%s unable to unprotect page. This may happen if you have SELinux"
					" enabled. Disable SELinux execmod protection for mysqld. Aborting."
					" Page: %p, Size: %zu, errno: %d.",
					log_prefix, (void *)addr, len, errno);
				return res;
			}
			res = protect(addr, len);
			elog(DEBUG3, "%s protect res: %d", log_prefix, res);
			if (res)
			{
				elog(ERROR, "%s unable to protect page. This may happen if you have SELinux enabled."
					" Disable SELinux execmod protection for mysqld. Aborting. Page: "
					"%p, Size: %zu, errno: %d.", log_prefix, (void *)addr, len, errno);
				return res;
			}
		}
		else // all is good
	...
	{
		elog(ERROR, "%s unable to unprotect. Page: %p, Size: %zu, errno: %d. Error.",
			log_prefix, (void *)addr, len, errno);
	}
	return res;
}

// macro to log via elog only if cond test is enabled
#define cond_info_print(cond_test, ...) do { if (cond_test) elog(DEBUG3, __VA_ARGS__);} while (0)
...
#define ASM_MODE 64
	...
	{
		elog(ERROR, "%s unable to unprotect trampoline function page: %p. Aborting.",
	...
	while (ud_disassemble(&ud_obj))
	{
		...
		{
			elog(ERROR, "%s unable to disassemble at address: %p. Aborting.",
				...
				ud_obj.operand[0].type == UD_OP_JIMM)
		{
			...
			if (*pc == 0xe8)
			{
				elog(DEBUG3, "define __i386 and pc ==0xe8");	 
		...
		else
		{
			...
			elog(DEBUG3, "uCurrentSize %d and InstrSize %d", uCurrentSize, InstrSize );	 

		}

		if (InstrSize >= JUMP_SIZE) // we have enough space so break
		{
			elog(DEBUG3, "have enough space so break ");	 
		...
		if (InstrSize >= JUMP32_SIZE)
		{
			...
			{
				disassemble_valid = true;
				elog(DEBUG3, "jumpType is Jump32");	 
			...
			{
				disassemble_valid = true;
				elog(DEBUG3, "jumpType is  IndirectJump");	 
	...
	{
		elog(ERROR, "%s unable to protect page. Error. Page: %p.", 
			 log_prefix, (void *)trampolineFunctionPage);
		return false;
	}
	...
	elog(DEBUG3, "WriteJump and uCurrentSize %d, InstrSize %d", uCurrentSize, InstrSize);	 

	*usedsize = uCurrentSize + JUMP_SIZE;
#ifndef __x86_64__
	elog(DEBUG3, "WriteJump in ifndef _x86_64");
	WriteJump((void *) targetFunction, newFunction);
#else
	elog(DEBUG3, "WriteJump %d above switch", jumpType);
	switch (jumpType) {
		ULONG_PTR addr;
	case Jump64:
		WriteJump((void *)targetFunction, newFunction);
	...
	case Jump32:
		...
		break;
	case IndirectJump:
		elog(DEBUG3, "WriteJump %d in indirectJump", jumpType);
...
static void UnhookFunction(ULONG_PTR Function, ULONG_PTR trampolineFunction, unsigned int trampolinesize)
{
	...
	{
		elog(ERROR, "%s Unhook not able to unprotect function page: %p. Aborting.",
 ...
 */
int hot_patch_function(void *targetFunction, void *newFunction, void *trampolineFunction, 
						unsigned int *trampolinesize, unsigned int *usedsize, int info_print)
{
	if (PAGE_SIZE ==0)
		PAGE_SIZE = GETPAGESIZE() ;

	DATATYPE_ADDRESS trampolinePage = get_page_address(trampolineFunction);
	cond_info_print(info_print > 0 ? true : false, "%s hot patching function: %p, trampolineFunction: %p trampolinePage: %p",
					log_prefix, (void *)targetFunction, (void *)trampolineFunction, (void *)trampolinePage);
 ...
 */
void remove_hot_patch_function(void *targetFunction, void *trampolineFunction,
								unsigned int trampolinesize, int info_print)
{
	if (trampolinesize == 0)
	{
	...
	cond_info_print(info_print>0 ? true : false, 
					"%s removing hot patching function: %p targetPage: %p trampolineFunction: %p",
					log_prefix, (void *)targetFunction, (void *)targetPage, (void *)trampolineFunction);
