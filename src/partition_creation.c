...
static Oid create_single_fdw_partition_internal(Oid parent_relid,
												RangeVar *partition_rv,
												char *tablespace,
												char *servername,
												char *remote_schema);
...
static char *choose_hash_partition_name(Oid parent_relid, Oid parent_nsp, uint32 part_idx);
...
static Node *build_partitioning_expression(Oid parent_relid,
										   Oid *expr_type,
										   List **columns);
static ObjectAddress create_foreign_table_using_stmt(CreateForeignTableStmt *create_fdw_stmt,
						     Oid relowner);

static Node *build_remote_raw_end_hash_check_tree(Node *raw_expression, uint32 end_value,
							Oid value_type, NEED_OP which_op);

/*
 * ---------------------------------------
 *  Public interface (fdw partition creation)
 * ---------------------------------------
 */

/* Create one fdw RANGE partition [start_value, end_value) */
Oid
create_single_fdw_range_partition_internal(Oid parent_relid,
										   ...
										   char *remote_schema)
{
	...
	if (!partition_rv)
	{
        ...
        if (strlen(partition_name)>63)
        {
            elog(WARNING, "table's name is too long, child table's name is truncated!");
        }
		partition_rv = makeRangeVar(PARTITION_TABLE_SCHEMA, partition_name, -1);
	}
	...
	partition_relid = create_single_fdw_partition_internal(parent_relid,
															partition_rv,
															tablespace,
															server_name,
															remote_schema);
									  ...
									  char *tablespace)
{
	...
	if (!partition_rv)
	{
		...
		partition_name = choose_hash_partition_name(parent_relid, parent_nsp, part_idx);
...
/* Create one fdw HASH partition */
Oid
create_single_fdw_hash_partition_internal(Oid parent_relid,
									  uint32 part_idx,
									  uint32 part_count,
									  RangeVar *partition_rv,
									  char *tablespace,
									  char *remote_schema)
{
	Oid			partition_relid, expr_type;
	Constraint		*check_constr;
	init_callback_params	callback_params;
	List			*trigger_columns = NIL;
	Node			*expr;
	char 			*server;
	int			part_per_server = part_count/rangeServerSet->server_count;

	if ((part_count % rangeServerSet->server_count) != 0)
		part_per_server++;

	server = rangeServerSet->server_set[part_idx/part_per_server].server_name;
	/* Generate a name if asked to */
	if (!partition_rv)
	{
		Oid		parent_nsp = get_rel_namespace(parent_relid);
	...
	/* Build check constraint for HASH partition */
	check_constr = build_remote_hash_check_constraint(partition_relid,
											   expr,
											   part_idx,
											   part_count,
											   expr_type);
					 ...
					 Oid collid)					/* collation id */
{
	...
	bool		create_fdw = false;
	char		*ns_name = NULL, *rel_name = NULL, 
				*remote_schema = NULL,
				**server_array = NULL;
	int			server_total = 0;
	Datum   	values[Natts_table_partition_rule];
	bool    	isnull[Natts_table_partition_rule];
	StringInfoData table_def;
	Relation 	parent_rel  = heap_open(parent_relid, AccessShareLock);

	ns_name = get_namespace_name(RelationGetNamespace(parent_rel));
	rel_name = pstrdup(RelationGetRelationName(parent_rel));
	heap_close(parent_rel, AccessShareLock);
	create_fdw =  read_table_partition_rule_params(ns_name, rel_name, values, isnull);

	if (create_fdw){
		 if (!isnull[Anum_table_partition_rule_remote_schema-1]) {
                	remote_schema = TextDatumGetCString(
					values[Anum_table_partition_rule_remote_schema-1]);
        	} else 
                	remote_schema = "public";

		initStringInfo(&table_def);
		buildCTS4RemoteServer(&table_def, parent_relid);
		server_array = getRemoteServer4Child(&server_total);
		Assert(server_array != NULL);
				...
				check_lt(&cmp_value_bound_finfo, collid, value, cur_leading_bound))
	{
		...
		if (create_fdw) {
			...
			UserMapping *user;
			Relation  child_rel;
			char * server_name = NULL, *remote_table_name = NULL;
			ListCell   *option;
			server_name = server_array[random()%server_total];
			Assert(server_name != NULL);
			last_partition = create_single_fdw_range_partition_internal(
										parent_relid,
										&bounds[0],
										&bounds[1],
										range_bound_type,										     NULL, NULL,
										server_name,
										remote_schema);
			initStringInfo(&sql);
			ftable = GetForeignTable(last_partition);
			foreach(option, ftable->options) {
				DefElem    *def = (DefElem *) lfirst(option);
				if (strcmp(def->defname, "table_name") == 0) {
					remote_table_name = defGetString(def);
					break;
				}
			}
			Assert(remote_table_name != NULL);
			child_rel = heap_open(ftable->relid, AccessShareLock);
			user = GetUserMapping(child_rel->rd_rel->relowner, ftable->serverid);
			heap_close(child_rel, AccessShareLock);
			appendStringInfo(&sql, "create table %s.%s %s;", remote_schema,
							 remote_table_name, table_def.data);
			buildCIS4RemoteServer(&sql, parent_relid, remote_schema, remote_table_name);
			connectionPoolRunSQL(user, sql.data, true);
		} else {
			last_partition = create_single_range_partition_internal(
										parent_relid,
										&bounds[0],
										&bounds[1],
										range_bound_type,										     NULL, NULL);
...
choose_range_partition_name(Oid parent_relid, Oid parent_nsp)
{
	...
	while (true)
	{
		...
		relname = psprintf("_%s_" UINT64_FORMAT "_%s" ,
						   get_namespace_name(parent_nsp),
						   (uint64) DatumGetInt64(part_num),
						   get_rel_name(parent_relid)
						   ); /* can't use UInt64 on 9.5 */
...
static char *
choose_hash_partition_name(Oid parent_relid, Oid parent_nsp, uint32 part_idx)
{
	return psprintf("_%s_%u_%s", get_namespace_name(parent_nsp),
			part_idx, get_rel_name(parent_relid));
}
...
/* Create a partition-like fdw table (no constraints yet) */
static Oid
create_single_fdw_partition_internal(Oid parent_relid,
									 RangeVar *partition_rv,
									 char *tablespace,
									 char *servername,
									 char *remote_schema)
{
	...
	/* Parent's namespace and name */
	Oid			parent_nsp;
	char			*parent_name, *parent_nsp_name;
	...
	CreateStmt		*create_stmt;
	CreateForeignTableStmt	create_fdw_stmt;
	DefElem *schema_option, *table_option;
	List			*create_stmts;
	ListCell		*lc;
	TupleDesc		tupleDesc;
        TupleConstr		*constr;
	...
	/* Initialize CreateForeignTableStmt structure */
	NodeSetTag(&create_fdw_stmt, T_CreateForeignTableStmt);
	create_stmt = &(create_fdw_stmt.base);
	...
	create_stmt->partition_info	= NULL;
#endif
	create_fdw_stmt.servername	= servername;
	schema_option = makeDefElem("schema_name",(Node *)makeString(remote_schema)
#if PG_VERSION_NUM >= 100000
					, -1
#endif
					);

	{
		/*StringInfoData tmp;
		initStringInfo(&tmp);
		appendStringInfo(&tmp, "cs_remote%s", partition_rv->relname);
        table_option = makeDefElem("table_name", (Node *)makeString(tmp.data)*/
        table_option = makeDefElem("table_name", (Node *)makeString(partition_rv->relname)
#if PG_VERSION_NUM >= 100000
					, -1
#endif
					);
	...
	create_fdw_stmt.options	= list_make2(schema_option, table_option);
	...
	parent_relation = relation_openrv(parent_rv, AccessShareLock);
        tupleDesc = RelationGetDescr(parent_relation);
	...
	for (parent_attno = 1; parent_attno <= tupleDesc->natts; parent_attno++)
		...
		Form_pg_attribute attribute = &(tupleDesc->attrs[parent_attno - 1]);
 		...
 		create_stmt->tableElts = lappend(create_stmt->tableElts, def);
	...
	/* Obtain the sequence of Stmts to create partition and link it to parent */
	create_stmts = transformCreateStmt((CreateStmt *)(&create_fdw_stmt), NULL);
	/* Create the partition and all required relations */
	foreach (lc, create_stmts)
	{
		Node *cur_stmt;
		...
		else if (IsA(cur_stmt, CreateForeignTableStmt))
		{
			...
			/* Create a partition and save its Oid */
			partition_relid = create_foreign_table_using_stmt((CreateForeignTableStmt *) cur_stmt,
								  child_relowner).objectId;
...
/* Create a new foreign table using cooked CreatefdwStmt */
static ObjectAddress
create_foreign_table_using_stmt(CreateForeignTableStmt *create_fdw_stmt, Oid relowner)
{
	...
	table_addr = DefineRelationCompat((CreateStmt*)create_fdw_stmt, RELKIND_FOREIGN_TABLE, relowner,  NULL);

	CreateForeignTable((CreateForeignTableStmt *) create_fdw_stmt, table_addr.objectId);
	/* Save data about a simple DDL command that was just executed */
	EventTriggerCollectSimpleCommand(table_addr, InvalidObjectAddress, (Node *) create_fdw_stmt);
	/*
...
/* Build left or right (depends on @is_left)HASH check constraint expression tree */
static Node *
build_remote_raw_end_hash_check_tree(Node *raw_expression, uint32 end_value,
					Oid value_type, NEED_OP which_op)
{
	...
	part_count_c->val = make_int_value_struct(HASH_SLOT_SIZE);
	part_count_c->location = -1;

	part_idx_c->val = make_int_value_struct(end_value);
	...
	if (which_op == NEED_LAQ)
		eq_oper->name = list_make1(makeString(">="));
	else if (which_op == NEED_LET)
		eq_oper->name = list_make1(makeString("<"));
	else 
		eq_oper->name = list_make1(makeString("="));
...
/* Build remote HASH check constraint expression tree */
Node *
build_remote_raw_hash_check_tree(Node *raw_expression,
						  uint32 part_idx,
						  uint32 part_count,
						  Oid relid,
						  Oid value_type)
{
	int16		   hash_range_start = 0, hash_range_end = 0, 
				range_step = 0, part_per_server = 0;
	RangeServer	   *rangeServer;

	part_per_server = part_count/rangeServerSet->server_count;
	if ((part_count%rangeServerSet->server_count)!=0)
		part_per_server++;

	rangeServer = rangeServerSet->server_set + (part_idx/part_per_server);
	range_step = rangeServer->hash_range_end - rangeServer->hash_range_start;
	if (range_step % part_per_server == 0) {
		range_step /= part_per_server;
	} else {
		range_step /= part_per_server;
		range_step++;
	}
	...
	Assert (range_step > 0);
	hash_range_start = rangeServer->hash_range_start + 
				(range_step * (part_idx%part_per_server));
	hash_range_end = range_step + hash_range_start;
	if (hash_range_end > rangeServer->hash_range_end)
		hash_range_end = rangeServer->hash_range_end;

	return (Node *) makeBoolExpr(AND_EXPR, 
				 list_make2(build_remote_raw_end_hash_check_tree(raw_expression, hash_range_start, 
											value_type, NEED_LAQ),
				build_remote_raw_end_hash_check_tree(raw_expression, hash_range_end, 
											value_type, NEED_LET)),
				     -1);
}
							...
							Oid value_type)
{
	...
	Node		*node = NULL;
	if (part_count > HASH_SLOT_SIZE) {
		elog(ERROR, "Cannot create hash partitions more than %d", HASH_SLOT_SIZE);
	}

	range_step = HASH_SLOT_SIZE/part_count;
	if (HASH_SLOT_SIZE % part_count != 0) {
		range_step++;
	}

	hash_range_start =  range_step * part_idx;
	/* Build a correct name for this constraint */
	hash_constr_name = build_check_constraint_name_relid_internal(child_relid);
	hash_range_end = range_step + hash_range_start;
	if (hash_range_end > HASH_SLOT_SIZE)
		hash_range_end =  HASH_SLOT_SIZE;

	node = (Node *) makeBoolExpr(AND_EXPR, 
				list_make2(build_remote_raw_end_hash_check_tree(raw_expression, hash_range_start, 
											value_type, NEED_LAQ),
				build_remote_raw_end_hash_check_tree(raw_expression, hash_range_end, 
											value_type, NEED_LET)),
	...
	/* Initialize basic properties of a CHECK constraint */
	hash_constr = make_constraint_common(hash_constr_name, node);
	return hash_constr;
}

/* Build complete remote HASH check constraint */
Constraint *
build_remote_hash_check_constraint(Oid child_relid,
							Node *raw_expression,
							uint32 part_idx,
							uint32 part_count,
							Oid value_type)
{
	...
	hash_constr = make_constraint_common(hash_constr_name,
										 build_remote_raw_hash_check_tree(raw_expression,
																   part_idx,
																   part_count,
																   child_relid,
																   value_type));
									  ...
									  List *columns)
{
	...
	func = list_make2(makeString(get_namespace_name(get_pathman_schema())),
					  makeString(CppAsString(gogudb_update_trigger_func)));
