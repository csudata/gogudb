 ...
 */
void
GoguclassifyConditions(PlannerInfo *root,
				   RelOptInfo *baserel,
				   List *input_conds,
				   List **remote_conds,
				   List **local_conds)
{
	...
	*local_conds = NIL;
	...
	foreach(lc, input_conds)
	{
		...
		if (Gogu_is_foreign_expr(root, baserel, ri->clause))
 ...
 */
bool
Gogu_is_foreign_expr(PlannerInfo *root,
				RelOptInfo *baserel,
				Expr *expr)
{
	foreign_glob_cxt glob_cxt;
					...
					foreign_loc_cxt *outer_cxt)
{
		...
		case T_FuncExpr:
			{
				 ...
				 */
				if (!Gogu_is_shippable(fe->funcid, ProcedureRelationId, fpinfo))
					return false;
		...
		case T_DistinctExpr:	/* struct-equivalent to OpExpr */
			{
				 ...
				 * function is too.)
				 */
				if (!Gogu_is_shippable(oe->opno, OperatorRelationId, fpinfo))
					return false;
		...
		case T_Aggref:
			{
				...
				if (!Gogu_is_shippable(agg->aggfnoid, ProcedureRelationId, fpinfo))
					return false;
				...
				if (agg->aggorder)
				{
					...
					foreach(lc, agg->aggorder)
					{
						...
						if (srt->sortop != typentry->lt_opr &&
							...
							!Gogu_is_shippable(srt->sortop, OperatorRelationId,
										  fpinfo))
							return false;
					}
				}
	 ...
	 */
	if (check_type && !Gogu_is_shippable(exprType(node), TypeRelationId, fpinfo))
		return false;
 ...
 */
List *
Gogu_build_tlist_to_deparse(RelOptInfo *foreignrel)
{
	List	   *tlist = NIL;
 ...
 */
void
GogudeparseSelectStmtForRel(StringInfo buf, PlannerInfo *root, RelOptInfo *rel,
...
const char *
Gogu_get_jointype_name(JoinType jointype)
{
	switch (jointype)
	{
		case JOIN_INNER:
					  ...
					  List **params_list)
{
	...
	if (IS_JOIN_REL(foreignrel))
	{
		...
		appendStringInfo(buf, "(%s %s JOIN %s ON ", join_sql_o.data,
						 Gogu_get_jointype_name(fpinfo->jointype), join_sql_i.data);
				   ...
				   List **params_list)
{
	...
	if (make_subquery)
	{
		...
		appendStringInfoChar(buf, '(');
		GogudeparseSelectStmtForRel(buf, root, foreignrel, NIL,
 ...
 */
void
GogudeparseInsertSql(StringInfo buf, RangeTblEntry *rte,
				 Index rtindex, Relation rel,
 ...
 */
void
GogudeparseUpdateSql(StringInfo buf, RangeTblEntry *rte,
				 Index rtindex, Relation rel,
 ...
 */
void
GogudeparseDirectUpdateSql(StringInfo buf, PlannerInfo *root,
					   Index rtindex, Relation rel,
 ...
 */
void
GogudeparseDeleteSql(StringInfo buf, RangeTblEntry *rte,
				 Index rtindex, Relation rel,
				 List *returningList,
				 List **retrieved_attrs)
{
 ...
 */
void
GogudeparseDirectDeleteSql(StringInfo buf, PlannerInfo *root,
					   Index rtindex, Relation rel,
					   RelOptInfo *foreignrel,
					   List *remote_conds,
					   List **params_list,
 ...
 */
void
GogudeparseAnalyzeSizeSql(StringInfo buf, Relation rel)
{
	...
	GogudeparseStringLiteral(buf, relname.data);
 ...
 */
void
GogudeparseAnalyzeSql(StringInfo buf, Relation rel, List **retrieved_attrs)
{
 ...
 */
void
GogudeparseStringLiteral(StringInfo buf, const char *val)
{
	const char *valptr;
...
deparseConst(Const *node, deparse_expr_cxt *context, int showtype)
{
	...
	switch (node->consttype)
	{
		...
		case BOOLOID:
			...
			break;
		default:
			GogudeparseStringLiteral(buf, extval);
			break;
	}
...
appendOrderByClause(List *pathkeys, deparse_expr_cxt *context)
{
	...
	foreach(lcell, pathkeys)
	{
		...
		em_expr = Gogu_find_em_expr_for_rel(pathkey->pk_eclass, baserel);
		Assert(em_expr != NULL);
