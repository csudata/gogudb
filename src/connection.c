...
typedef struct ConnCacheEntry
{
	...
	bool		not_auto_commit;	/* need to commit at clean up call back ? */		
} ConnCacheEntry;
 ...
 */
PGconn *
GoguGetConnection(UserMapping *user, bool will_prep_stmt, bool inXact)
{
	bool		found;
	...
	if (inXact) {
		...
		entry->not_auto_commit = true;
	}
	else 
		entry->not_auto_commit = false;
...
connect_pg_server(ForeignServer *server, UserMapping *user)
{
	...
	PG_TRY();
	{
		...
		n = 0;
		n += GoguExtractConnectionOptions(server->options,
									  keywords + n, values + n);
		n += GoguExtractConnectionOptions(user->options,
									  keywords + n, values + n);
...
do_sql_command(PGconn *conn, const char *sql)
{
	...
	if (!PQsendQuery(conn, sql))
		Gogu_pgfdw_report_error(ERROR, NULL, conn, false, sql);
	res = Gogu_pgfdw_get_result(conn, sql);
...
begin_remote_xact(ConnCacheEntry *entry)
{
	...
	if (entry->xact_depth <= 0)
	{
        ...
        */
        /* tangcheng 2018.07.26  transaction isolation can not high, because sysbench test failed */
        	sql = "BEGIN";
 ...
 */
void
GoguReleaseConnection(PGconn *conn)
{
	/*
 ...
 */
unsigned int
GoguGetCursorNumber(PGconn *conn)
{
 ...
 */
unsigned int
GoguGetPrepStmtNumber(PGconn *conn)
{
 ...
 */
PGresult *
Gogu_pgfdw_exec_query(PGconn *conn, const char *query)
{
	/*
	...
	if (!PQsendQuery(conn, query))
		Gogu_pgfdw_report_error(ERROR, NULL, conn, false, query);

	/* Wait for the result. */
	return Gogu_pgfdw_get_result(conn, query);
}
 ...
 */
PGresult *
Gogu_pgfdw_get_result(PGconn *conn, const char *query)
{
 ...
 */
void
Gogu_pgfdw_report_error(int elevel, PGresult *res, PGconn *conn,
...
pgfdw_exec_cleanup_query(PGconn *conn, const char *query, bool ignore_errors)
{
	...
	if (!PQsendQuery(conn, query))
	{
		Gogu_pgfdw_report_error(WARNING, NULL, conn, false, query);
		return false;
	}
	...
	{
		Gogu_pgfdw_report_error(WARNING, result, conn, true, query);
...
void connectionPoolRunSQL(UserMapping *user, const char *query, bool inXact)
{
	PGconn *conn = GoguGetConnection(user, false, false);
	if (inXact)
		do_sql_command(conn, "BEGIN");
	do_sql_command(conn, query);
	if (inXact)
		do_sql_command(conn, "COMMIT TRANSACTION");
