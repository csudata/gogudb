...
bg_worker_load_config(const char *bgw_name)
{
	/* Try to load config */
	if (!load_config())
		elog(ERROR, "%s: could not load extension's config [%u]",
			 bgw_name, MyProcPid);
	else
		elog(LOG, "%s: loaded extension's config [%u]",
			 bgw_name, MyProcPid);
}
...
#define HandleError(condition, new_state) \
	...
	snprintf(worker.bgw_library_name, BGW_MAXLEN, "gogudb");
...
handle_exec_state:
	...
	switch (exec_state)
	{
		...
		case BGW_PM_DIED:
			ereport(ERROR,
					(errmsg("Postmaster died during the gogudb background worker process"),
...
bgw_main_spawn_partitions(Datum main_arg)
{
	...
	BackgroundWorkerInitializeConnectionByOid(args->dbid, args->userid, 0);
#else
...
bgw_main_concurrent_part(Datum main_arg)
{
	...
	/* Establish connection and start transaction */
#if PG_VERSION_NUM >= 110000
	BackgroundWorkerInitializeConnectionByOid(part_slot->dbid, part_slot->userid, 0);
#else
