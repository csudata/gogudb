 ...
 */
PG_FUNCTION_INFO_V1(gogudb_fdw_validator);

Datum
gogudb_fdw_validator(PG_FUNCTION_ARGS)
{
	...
	foreach(cell, options_list)
	{
		...
		else if (strcmp(def->defname, "extensions") == 0)
		{
			...
			(void) GoguExtractExtensionList(defGetString(def), true);
		}
 ...
 */
int
GoguExtractConnectionOptions(List *defelems, const char **keywords,
						 const char **values)
{
	ListCell   *lc;
	int			i;
 ...
 */
List *
GoguExtractExtensionList(const char *extensionsString, bool warnOnMissing)
{
