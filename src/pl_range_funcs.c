...
PG_FUNCTION_INFO_V1( create_remote_range_partitions_internal );
...
Datum
create_remote_range_partitions_internal(PG_FUNCTION_ARGS)
{
	Oid				parent_relid;
	int16			typlen;
	bool			typbyval;
	...
	if (!PG_ARGISNULL(4)) {
		server_names = deconstruct_text_array(PG_GETARG_DATUM(4), &nservers);
	} 

	if (nservers == 0) {
		ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
						errmsg("cannot find available postgres_fdw server"),
						errdetail("Please install postgres_fdw and create remote server")));
	}
	...
	for (i = 0; i < ndatums - 1; i++)
	{
		...
		char	   *server_name = server_names[i%nservers];
		(void) create_single_fdw_range_partition_internal(parent_relid,
													  &start,
													  &end,
													  bounds_type,
													  name,
