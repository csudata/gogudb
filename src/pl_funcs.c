...
PG_FUNCTION_INFO_V1( add_to_gogudb_config );
PG_FUNCTION_INFO_V1( gogudb_config_params_trigger_func );
...
show_partition_list_internal(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		tuptab_mcxt = AllocSetContextCreate(CurrentMemoryContext,
											"tuptable for gogudb_partition_list",
 ...
 */
Datum
add_to_gogudb_config(PG_FUNCTION_ARGS)
{
	Oid					relid;
	char			   *expression;
	PartType			parttype;
	...
	switch (PG_NARGS())
	{
		...
		default:
			elog(ERROR, "error in function " CppAsString(add_to_gogudb_config));
 ...
 */
Datum
gogudb_config_params_trigger_func(PG_FUNCTION_ARGS)
{
...
Datum
gogudb_update_trigger_func(PG_FUNCTION_ARGS)
{
	TriggerData			   *trigdata = (TriggerData *) fcinfo->context;
...
#if PG_VERSION_NUM >= 110000
		char	   *attname = get_attname(parent_relid, attnum, false);
#else
...
/* NOTE: just in case */
Datum
get_gogudb_lib_version(PG_FUNCTION_ARGS)
{
