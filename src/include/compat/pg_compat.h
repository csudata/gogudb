...
#include "pg_config.h"
	...
	#error "Cannot build gogudb with PostgreSQL version lower than 9.5.3"
#elif PG_VERSION_NUM < 90504
	#warning "It is STRONGLY recommended to use gogudb with PostgreSQL 9.5.4 since it contains important fixes"
#endif
...
#define create_append_path_compat(rel, subpaths, required_outer, parallel_workers) \
		create_append_path((root), (rel), (subpaths), NIL, (required_outer), (parallel_workers), false, NIL, -1)
#else
#define create_append_path_compat(rel, subpaths, required_outer, parallel_workers) \
		create_append_path((root), (rel), (subpaths), NIL, (required_outer), (parallel_workers), NIL, \
						   false, false, NIL, -1)
#endif /* PGPRO_VERSION */
