...
Oid create_single_range_partition_internal(Oid parent_relid,
										   ...
										   char *tablespace);
/* Create one RANGE partition */
Oid create_single_fdw_range_partition_internal(Oid parent_relid,
										   const Bound *start_value,
										   const Bound *end_value,
										   Oid value_type,
										   RangeVar *partition_rv,
...
Oid create_single_fdw_hash_partition_internal(Oid parent_relid,
										  uint32 part_idx,
										  uint32 part_count,
										  RangeVar *partition_rv,
										  char *tablespace,
...
Node * build_remote_raw_hash_check_tree(Node *raw_expression,
								 uint32 part_idx,
								 uint32 part_count,
								 Oid relid,
								 Oid value_type);

Constraint * build_remote_hash_check_constraint(Oid child_relid,
										 Node *raw_expression,
										 uint32 part_idx,
										 uint32 part_count,
										 Oid value_type);
